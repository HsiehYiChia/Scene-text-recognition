...
int main(int argc, char** argv)
{
	...
	Mat test_img;
	VideoWriter writer;
	cap >> test_img;	// get 1 frame to know the frame size
	writer.open("result.wmv", CV_FOURCC('W', 'M', 'V', '2'), 10.0, test_img.size(), true);
	if (!writer.isOpened()) {
	...
	/*for (;;)
	{
		...
		vector<double> times = er_filter->text_detect(frame, root, all, pool, strong, weak, tracked, text);
		show_result(frame, result, all, pool, strong, weak, tracked, text, times);
	...
	#else
		time[0] = chrono::high_resolution_clock::now();
		er_filter->er_grouping(tracked_vec, text, false, true);
		time[1] = chrono::high_resolution_clock::now();
		er_filter->er_ocr(tracked_vec, channel_vec, text);
